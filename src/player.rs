...
async fn fetch_player_main_stats(data: &Value) -> Result<MainPlayerData> {
    ...
    let dungeons = parse_dungeon_data(Some(global_data_object["dungeons"].as_object().ok_or_else(|| {
        anyhow::Error::msg("Expected 'dungeons' to be an object")
    })?))?.ok_or_else(|| {
        anyhow::Error::msg("Global dungeons data should not be null")
    })?;

    let raids = parse_raid_data(Some(global_data_object["raids"].as_object().ok_or_else(|| {
        anyhow::Error::msg("Expected 'raids' to be an object")
    })?))?.ok_or_else(|| {
        anyhow::Error::msg("Raids data should not be null")
    })?;
...
async fn fetch_character_data(data: &Value) -> Result<HashMap<String, CharacterData>> {
    ...
    let characters_object = player_object.get("characters").ok_or_else(|| {
        anyhow::Error::msg("Expected 'characters' to be a JSON object")
    })?.as_object().unwrap();

    let mut characters: HashMap<String, CharacterData> = HashMap::new();

    for (character, character_details) in characters_object {
        let class = character_details["type"].as_str().ok_or_else(|| {
            anyhow::Error::msg("Expected 'type' to be a String")
        })?.parse::<String>().unwrap();

        let nickname = character_details["nickname"].as_str().map(|s| s.to_string());

        let level = character_details["level"].as_i64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'level' to be an integer")
        })? as i32;

        let xp = character_details["xp"].as_i64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'xp' to be an integer")
        })?;

        let xp_percent = character_details["xpPercent"].as_i64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'xpPercent' to be an integer")
        })? as i32;

        let total_level = character_details["totalLevel"].as_i64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'totalLevel' to be an integer")
        })? as i32;

        let wars = character_details["wars"].as_i64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'wars' to be an integer")
        })? as i32;

        let playtime = character_details["playtime"].as_f64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'playtime' to be an integer")
        })? as f32;

        let mobs_killed = character_details["mobsKilled"].as_i64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'mobsKilled' to be an integer")
        })? as i32;

        let chests_found = character_details["chestsFound"].as_i64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'chestsFound' to be an integer")
        })? as i32;

        let items_identified = character_details["itemsIdentified"].as_i64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'itemsIdentified' to be an integer")
        })? as i32;

        let blocks_walked = character_details["blocksWalked"].as_i64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'blocksWalked' to be an integer")
        })?;

        let logins = character_details["logins"].as_i64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'logins' to be an integer")
        })? as i32;

        let deaths = character_details["deaths"].as_i64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'deaths' to be an integer")
        })? as i32;

        let discoveries = character_details["discoveries"].as_i64().ok_or_else(|| {
            anyhow::Error::msg("Expected 'discoveries' to be an integer")
        })? as i32;

        let pre_economy = character_details["preEconomy"].as_bool().ok_or_else(|| {
            anyhow::Error::msg("Expected 'preEconomy' to be a bool")
        })?;

        let pvp_object = character_details["pvp"].as_object().ok_or_else(|| {
            anyhow::Error::msg("Expected 'pvp' to be an object")
        })?;

        let pvp = create_pvp_data(pvp_object)?;

        let gamemode_array = character_details["gamemode"].as_array().expect("Expected gamemode data to be a JSON array");

        let gamemodes: Vec<String> = gamemode_array
            .iter()
            .map(|gamemode_value| gamemode_value.as_str().expect("Expected gamemode value to be a string").to_string())
            .collect();

        let skill_points = character_details["skillPoints"].as_object().map(|obj| {
            SkillPointData {
                ...
                intelligence: obj.get("intelligence").and_then(|v| v.as_i64()).unwrap_or(0) as i32,
        ...
        let quests_array = character_details["quests"].as_array().expect("Expected quests data to be a JSON array");

        let quests: Vec<String> = quests_array
            .iter()
            .map(|quest_value| quest_value.as_str().expect("Expected quest value to be a string").to_string())
            .collect();

        characters.insert(character.to_string(), CharacterData {
            class,
            nickname,
            level,
            xp,
...
pub async fn get_player_full_stats(identifier: &str) -> Result<FullPlayerData> {
    let url = format!("https://api.wynncraft.com/v3/player/{}?fullResult=True", identifier);
...
fn parse_list_to_hashmap(list_object: &Map<String, Value>) -> Result<HashMap<String, i32>, anyhow::Error> {
...
fn parse_dungeon_data(dungeons_object: Option<&Map<String, Value>>) -> Result<Option<DungeonData>, anyhow::Error> {
    match dungeons_object {
        Some(dungeons) => {
            let total = dungeons["total"].as_i64().ok_or_else(|| {
                anyhow::Error::msg("Expected 'total' to be an i64")
            })? as i32;
            let dungeon_list = parse_list_to_hashmap(dungeons["list"].as_object().ok_or_else(|| {
                anyhow::Error::msg("Expected 'list' to be an object")
            })?)?;
            Ok(Some(DungeonData { total, dungeon_list }))
        },
        None => Ok(None),
    }
}

fn parse_raid_data(raids_object: Option<&Map<String, Value>>) -> Result<Option<RaidData>, anyhow::Error> {
    match raids_object {
        Some(raids) => {
            let total = raids["total"].as_i64().ok_or_else(|| {
                anyhow::Error::msg("Expected 'total' to be an i64")
            })? as i32;
            let raid_list = parse_list_to_hashmap(raids["list"].as_object().ok_or_else(|| {
                anyhow::Error::msg("Expected 'list' to be an object")
            })?)?;
